...
>(
  ...
  const sourceSql = foreignDataFormat.sql.selectSqlBase
  return `${sourceSql} where ${foreignColumnName} = $1 limit 1`
}
...
>(
  ...
  return `${sourceSql} where ${foreignColumnName} = $1`
}
...
where ${localTableName}.${localColumnName} = $1`
  )
}
